...
- 总结
>用户调用exists注册监听后，会做几个事情：
>1.将请求数据封装为packet，添加到outgoingQueue；  
>2. SendThread线程会执行数据发送，将outgoingQueue队列中的数据发送到服务端；  
>3. 通过clientCnxnSocket.doTransport(to,pendingQueue,ClientCnxn.this); 其中ClientCnxnSocket只zookeeper客户端和服务端的连接通信的封装，有两个具体的实现
类ClientCnxnSocketNetty和ClientCnxnSocketNIO;具体使用哪个类实现发送，是在初始化过程在实例化Zookeeper时设置的；  
>4. 基于第3步，最终在ClientCnxnSocketNetty方法中执行sendPkt将请求数据包发送到服务端。  
                ...
                if(this.bb != null) {
                   ...
                   // 将message写入bb中 message.readBytes(this.bb);
                    ...
                    if(e2 != null && e2.isRunning()) {
                        if(this.initialized) {
                            // 处理客户端传过来的数据包 e2.processPacket(this, this.bb);
